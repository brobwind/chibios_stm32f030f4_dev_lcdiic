...
#define _pcf8574_methods \
    msg_t (*set_port)(void *instance, uint8_t val); \
    msg_t (*get_port)(void *instance, uint8_t *val);
...
#define pcf8574SetPort(ip, val) \
    (ip)->vmt->set_port(ip, val)

#define pcf8574GetPort(ip, val) \
    (ip)->vmt->get_port(ip, val)
